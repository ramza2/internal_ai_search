...
**Worker prep functions:** `enqueue_scan_job` inserts **`status='PENDING'`** rows (returns **`None`** if DDL/columns missing). **`dequeue_pending_job`** uses **`FOR UPDATE SKIP LOCKED`**, **`ORDER BY priority DESC, created_at ASC`**, then sets **`RUNNING`** with **`worker_id`** / **`heartbeat_at`**. **`mark_job_completed`**, **`mark_job_failed`**, **`mark_job_cancelled`**, and **`update_job_heartbeat`** support the polling worker. **`update_scan_job_progress`** updates counters / **`current_file_path`** and optionally **`heartbeat_at`**. **`is_cancel_requested`** reads **`cancel_requested`**.

**DB polling worker:** A separate process (**no** Celery/RQ) runs `python -m app.worker_main` from the `backend` directory. **`WORKER_ENABLED=false`** is the default so the FastAPI app never auto-starts this loop; CLI runs the worker anyway and logs **"Running worker from CLI"** (see `app/worker_main.py`). The worker dequeues **`PENDING`** jobs and runs **`job_runner.run_job`**. Handlers that already finalize **`scan_jobs`** via **`complete_scan_job`** / **`fail_scan_job`** / **`mark_job_cancelled`** return **`WorkerRunResult.finalized_by_handler=true`** so the worker loop does **not** call **`mark_job_completed`** / **`mark_job_failed`** again.

**Production worker job: `WEBDAV_SYNC_TREE`:** When a queued job has **`job_type=WEBDAV_SYNC_TREE`** and **`job_params`** does **not** set **`worker_test_mode=true`**, the worker runs **`run_webdav_recursive_sync_core`** with **`scan_job_id=job.id`** — the same WebDAV BFS / DB upsert / optional deleted-detection logic as the synchronous **`POST /api/data-sources/{id}/sync-tree`** route, which continues to call **`create_scan_job`** then the same core (no duplicate **`scan_jobs`** rows). **`PROCESS_PENDING_TEXT`**, documents, chunk, and embed job types are **not** executed by the worker yet.
...
**`cancel_requested`:** The core checks **`cancel_check`** before the WebDAV walk (after validation), after **`collect_tree`**, and every **50** items during the upsert loop; if set, the job is **`mark_job_cancelled`** and the DB transaction is rolled back when mid-batch. There is still **no** admin HTTP cancel API — flags must be set in DB manually for testing.
...
Synchronous **`POST /api/data-sources/{id}/sync-tree`** still uses **`create_scan_job`** → **`RUNNING`** and does **not** use **`enqueue_scan_job`**. **`PENDING`** queue jobs are created only via **`test-enqueue`**, **`sync-tree` enqueue**, or future admin job APIs.

**Worker process scope (not implemented):** Cancel/retry HTTP APIs, parent/child pipeline fan-out, stale **`RUNNING`** reclaim, Celery/RQ/Redis, and worker execution for non-**`WEBDAV_SYNC_TREE`** production jobs (except test flags).
