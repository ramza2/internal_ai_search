...
- `SYNC_TREE_EMERGENCY_MAX_ITEMS` — optional ops-only cap when `scan_scope=FULL` (default **`0`** = no emergency cap). Does not affect synchronous LIMITED `sync-tree`.
...
- **Operational note:** this endpoint runs synchronously inside the HTTP request, with `max_depth` / `max_items` for safety (LIMITED mode only).
- **`scan_scope`:** Query `scan_scope=FULL` is **rejected with HTTP 400** and the message *전체 저장소 처리는 백그라운드 파이프라인에서 실행해주세요.* Use worker **`POST /api/admin/jobs/sync-tree`**, **`POST /api/admin/pipeline-jobs`**, or the UI **검색 반영** modal (background / PIPELINE). See **`docs/07_아키텍처/전체저장소_처리모드.md`**.
- **FULL repository scan (worker / PIPELINE only):** `job_params.scan_scope=FULL` with `max_depth` / `max_items` omitted or `null` — no user-facing depth/item cap (optional env **`SYNC_TREE_EMERGENCY_MAX_ITEMS`** > 0 for ops-only emergency cap). `collect_tree` still loads the full collected list into memory before upsert (**TODO:** streaming / batched upsert for very large trees). Worker runs honor **`cancel_check`** and heartbeat like other long jobs.
...
**`reprocess_hwp_no_extractable_text`:** when `true`, **PENDING** plus **`hwp`** rows with `SKIPPED` / `NO_EXTRACTABLE_TEXT` are eligible (same query ordering — PENDING comes first). Does **not** re-run **COMPLETED** HWP.

**`only_reprocess_hwp_no_extractable_text`:** when `true`, **only** `hwp` + `SKIPPED` + `NO_EXTRACTABLE_TEXT` (no PENDING backlog, no `UNSUPPORTED_EXTENSION`). `limit=1` selects a skipped HWP, not a pending one. `include_extensions` omitted → effective **`hwp`** only; explicit list without `hwp` → **400**. Takes precedence over `reprocess_skipped` / `reprocess_hwp_no_extractable_text`. **Admin UI:** data-sources **검색 반영** modal → Step 3 **「추출되지 않은 HWP 다시 처리」** (dry_run target check + `POST /api/admin/jobs/process-pending-documents` enqueue only; not wired into default pipeline / full search-reflect job).

| Option | PENDING | SKIPPED UNSUPPORTED | SKIPPED HWP NO_TEXT |
|--------|---------|---------------------|---------------------|
| (default) | yes | no | no |
| `reprocess_skipped=true` | yes | yes (supported ext) | no |
| `reprocess_hwp_no_extractable_text=true` | yes | no | yes |
| `only_reprocess_hwp_no_extractable_text=true` | **no** | **no** | **yes only** |

Tiered 도입 **이전** `NO_EXTRACTABLE_TEXT` HWP는 **자동 갱신 없음** — 위 옵션으로만 재시도. 성공 후 **chunk / embedding 별도 Job**.
...
curl -X POST "http://localhost:8000/api/data-sources/{id}/process-pending-documents?dry_run=true&include_extensions=hwp&reprocess_hwp_no_extractable_text=true" \
  -H "Authorization: Bearer <admin-token>"

curl -X POST "http://localhost:8000/api/data-sources/{id}/process-pending-documents?limit=20&include_extensions=hwp&reprocess_hwp_no_extractable_text=true" \
  -H "Authorization: Bearer <admin-token>"

curl -X POST "http://localhost:8000/api/data-sources/{id}/process-pending-documents?dry_run=true&only_reprocess_hwp_no_extractable_text=true&limit=1" \
  -H "Authorization: Bearer <admin-token>"
```

- **Query parameters:** `limit`, `max_file_size_bytes`, optional `include_extensions`, `dry_run`, `reprocess_skipped`, `reprocess_hwp_no_extractable_text`, **`only_reprocess_hwp_no_extractable_text`** (default **false**).
- **Admin Job body** — same booleans plus `data_source_id`, `limit`, `priority`.
...
**Admin process-pending-documents enqueue (worker queue):** `POST /api/admin/jobs/process-pending-documents` — body fields above plus **`only_reprocess_hwp_no_extractable_text`**. **`job_params`** stores all three reprocess booleans. **`action_logs`** **`detail`** adds **`only_reprocess_hwp_no_extractable_text`** (booleans only).
...
- `POST /api/admin/jobs/process-pending-documents` — Body includes **`only_reprocess_hwp_no_extractable_text`**. Best-effort **`JOB_PROCESS_PENDING_DOCUMENTS_ENQUEUE`** in **`action_logs`**.
