...
**`action_logs`:** The three **GET** list/detail/failure routes are **not** appended to `action_logs` — they are frequent operational reads (same rationale as dashboard summary / search data-sources listing). **`POST …/cancel`**, **`POST …/sync-tree`**, **`POST …/process-pending-text`**, **`POST …/process-pending-documents`**, **`POST …/chunk-completed-text`**, **`POST …/embed-pending-chunks`** may append rows as described above.
