...
**Worker process scope (still not implemented):** Retry HTTP API, automatic retry after failure, parent/child pipeline fan-out, Celery/RQ/Redis, and worker execution for non-**`WEBDAV_SYNC_TREE`** production jobs (except test flags).
...
**`action_logs`:** The three **GET** list/detail/failure routes are **not** appended to `action_logs` — they are frequent operational reads (same rationale as dashboard summary / search data-sources listing). **`POST …/cancel`** and **`POST …/sync-tree`** may append rows as described above.
