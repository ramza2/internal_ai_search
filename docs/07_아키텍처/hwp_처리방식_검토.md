...
| 단계 | 상태 | 참고 |
|------|------|------|
| E2E 검증 (documents → search → preview) | **미검증** | [`hwp_e2e_검증계획.md`](./hwp_e2e_검증계획.md) |
| Runtime 점검 스크립트 | **도구 추가** | `tools/hwp_poc/check_hwp_runtime.py` |
| 운영 이미지 hwp5txt | **계획 수립** | [`hwp_운영이미지_반영계획.md`](./hwp_운영이미지_반영계획.md) — Dockerfile 미존재, 예시·검증 절차만 |
| AGPL 법무 검토 | **필요** | 완료 전 운영 활성화 금지 |
| Python 3.11/3.12 + requirements pin | **TODO** | PoC는 3.14 사용 사례 있음 |
| **표·양식 HWP 고도화** | **tiered 1차 구현** | [`hwp_표양식_추출고도화_결과.md`](./hwp_표양식_추출고도화_결과.md) — `HWP_EXTRACTION_STRATEGY=tiered`(기본): **hwp5html flatten → 품질 검사 → hwp5txt fallback**. `hwp5txt_only`로 롤백 가능. AGPL·compose E2E·metadata DB 저장은 미완 |

**정책 유지:** HWPX 우선 권장 · Automation/COM 금지 · OCR·PDF·상용 SDK는 범위 외.
...
### 명시적 HWP 재처리 (tiered 도입 후, 2026-05-21)

- **자동 재분석 없음:** parser 전략을 tiered로 바꿔도, 이미 `SKIPPED` / `NO_EXTRACTABLE_TEXT`인 HWP는 **서버 기동·일괄 Job으로 갱신되지 않음**.
- **COMPLETED HWP** (`hwp5txt` 등 이전 성공분)도 **자동 재추출·chunk 교체 없음**.
- 관리자만 **`reprocess_hwp_no_extractable_text=true`** 로 `process-pending-documents`(또는 admin Job) 요청 시, **`hwp` + SKIPPED + NO_EXTRACTABLE_TEXT** 만 대상.
- **`reprocess_skipped`**(UNSUPPORTED_EXTENSION)와 **의미·대상 분리** 유지.
- 재처리 성공 후 **chunk / embedding은 별도 Job** (자동 연쇄 없음).
- **`only_reprocess_hwp_no_extractable_text=true`:** PENDING backlog 없이 **SKIPPED HWP만** 복구 (PENDING 수천 건 환경에서 `reprocess_hwp_*`만으로는 SKIPPED가 뒤로 밀림).
- COMPLETED HWP 재추출은 여전히 범위 밖.
- 검증: [`hwp_skipped_재처리_e2e_검증결과.md`](./hwp_skipped_재처리_e2e_검증결과.md)
